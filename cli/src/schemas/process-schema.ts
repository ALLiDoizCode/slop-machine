 ...
 */
const Address43Schema = z
  .string()
  .length(43, 'Address must be exactly 43 characters')
  .regex(
    /^[a-zA-Z0-9_-]+$/,
    'Address must contain only alphanumeric, underscore, and dash characters'
  );
...
const TxIdSchema = z
  ...
  .regex(
    /^[a-zA-Z0-9_-]+$/,
    'TX ID must contain only alphanumeric, underscore, and dash characters'
  );
...
const PricingSchema = z
  .record(z.string().min(1, 'Action name cannot be empty'), AoTokenAmountSchema)
...
export const ProcessMetadataSchema = z.object({
  ...
  capabilities: z
    ...
    .optional()
    .default([])
    .refine((caps) => caps.every((c) => c.length >= 2 && c.length <= 50), {
...
export const EXAMPLE_PROCESS_METADATA: ProcessMetadata = {
  ...
  skills: ['react_security_txid_43_chars_example_here'],
